...
которое может быть обработано макросом в файле :bolditalicred:`rec.xlsm` (см. `Макрос протокола в Microsoft Excel`_).
...
5. :underlined:`СТАТИСТИКА`, где можно заменить индикатор на график измерений (см. `Вкладка "СТАТИСТИКА"`_).
