...
.. figure:: ./_static/DigitalIndicatorOuterView.png
   :alt: DigitalIndicatorOuterView
   :align: center
...
* двойным щелчком левой кнопки мыши по любому из датчиков в списке всех найденных в главном окне приложения.

.. note:: Окно индикатора откроется автоматически после запуска приложения и последующего поиска датчиков, если в предыдущем сеансе работы это окно индикатора использовалось и не было закрыто пользователем вручную.

|

Окно индикатора открывается в режиме отображения измеряемой величины со стрелочным или цифровым индикатором.
Цвет фона стрелочного индикатора :highlightedlightyellow:`светло-желтый`, если показания индикатора находятся в допуске заданных на вкладке :underlined:`ДОПУСК` значений
...
которое может быть обработано макросом в файле :bolditalic:`rec.xlsm` (см. `Макрос протокола в Microsoft Excel`_).

.. attention:: В режимах *max-min* и сортировки использование макроса недоступно.

В режиме *max-min* под индикатором отображаются предельные показания и размах показаний датчика.
Кнопки |старт| и |стоп| позволяют соответственно начать и закончить измерение детали.
...
1. :underlined:`ИЗМЕРЕНИЕ`, где устанавливается правило формирования показания индикатора на основе показаний датчиков по следующей формуле:
...
* Чекбокс **Автоматическое сохранение, мин** активирует режим автоматического сохранения измеренных значений в формате XLS или CSV, а поле рядом позволяет задать его периодичность.
* Кнопка **СОХРАНИТЬ ИЗМЕРЕНИЯ** позволяет сохранить отмеченные показания индикатора вручную в формате XLSX или CSV.
...
5. :underlined:`СТАТИСТИКА`, где можно заменить индикатор на график измерений (см. `Вкладка статистики`_).
...
.. _Вкладка статистики: ./Charts.html


.. |старт| image:: _static/play.png
.. |стоп| image:: _static/stop.png