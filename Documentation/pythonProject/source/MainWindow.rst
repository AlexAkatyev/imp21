...
* **Поиск датчиков** — запуск поиска датчиков.
* **Новый индикатор** — создание нового окна индикатора с пустой формулой.
...
У любого из найденных датчиков можно просмотреть параметры с помощью кнопки вывода информации |info| в соответветствующей строке.
Параметры отличаются от типа датчика, а также от способа его подключения к компьютеру.

 .. figure:: ./_static/IndicatorInfo.png
   :alt: IndicatorInfo
   :align: center

 .. figure:: ./_static/IndicatorInfo1.png
   :alt: IndicatorInfo
   :align: center

На последнем примере, где датчик подключен через USB порт напрямую к программе ИМП, представлена возможность изменения пользовательского имени датчика, а также адреса датчика для дальнейшего его использования в сети Modbus.
